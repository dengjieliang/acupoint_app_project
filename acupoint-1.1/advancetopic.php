<?php
	...
	$result2 = $con2->query("SELECT main_table.acupoint FROM `main_table` WHERE meridian = '$merid'");
	...
	for($i = 0;$i< count($order_acu);$i += 1){
		if(in_array($order_acu[$i], $in_merid)){
			$acu_point[$i] = $acu_point[$i] + 1;
			$plus[$i] = 2;
		}
	}
	...
	while($point > 0.5){
		for($pt = 0;$pt < count($acu_point);$pt+=1){
			if ($acu_point[$pt] == $point && $plus[$pt] == 0 && $control == 1){
				array_push($acu_result, $order_acu[$pt]);
			}
			else if($acu_point[$pt] == $point && $plus[$pt] == 1 && $control == 2){
				array_push($acu_result, $order_acu[$pt]);
			}
			else if($acu_point[$pt] == $point && $plus[$pt] == 2 && $control == 3){
				array_push($acu_result, $order_acu[$pt]);
			}
		}
		if($control == 1){
	...
	$con2->query("SET NAMES UTF8");
	for($now_dis=0;$now_dis<count($dis);$now_dis+=1){
		$result2 = $con2->query("SELECT `liketo` FROM `distancelike` WHERE symptom = '$dis[$now_dis]'");
		$row = $result2->fetchColumn();
		if ($row!=false){
			if ($acu_order[$acu_num] == ""){
